        ...
        for (int i = 0; i < Jogo.getObjJogo().size(); i++) {
            ...
            if (atual instanceof Fantasma && bonus) {
                if (Objetos.isColliding(this, atual)) {
                    this.score += 200;
                    Jogo.getObjJogo().remove(atual);
                    Jogo.getObjJogo().add(new Fantasma(160, 200, 16, 16, Objetos.FANTASMA_EN));
                }
                if(timer >= 80){
                    bonus = false;
                }
...
//        }
    ...
    public void render(Graphics g) {
        g.drawImage(Jogo.spritesheet.getSprite(32,0,16,16), this.getX() - Camera.x, this.getY() - Camera.y, null);
    }
